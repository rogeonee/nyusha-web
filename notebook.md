...
- **Reasoning:** Gemini thought summaries enabled for 3.1 Pro / 3.5 Flash (`includeThoughts: true` with `thinkingLevel`). 3.1 Flash-Lite is the stable low-cost fallback (`thinkingLevel: 'low'`, `includeThoughts: false`). `sendReasoning` defaults to true in AI SDK.
